...
import { buildPrintableAgendaHtml, downloadAgendaCsv, printAgendaWindow } from "@/lib/mylife/lib/agendaExport";
import type { AgendaExportPeriod } from "@/lib/mylife/lib/exportReports";
...
export function AgendaView() {
  ...
  return (
    <div className="space-y-5">
      ...
      <LifeModal open={exportOpen} onClose={() => setExportOpen(false)} title="Exporter l’agenda (PDF / Excel)">
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        <button
          type="button"
          className="flex-1 rounded-xl bg-primary py-2.5 text-sm font-semibold text-primary-foreground active:scale-[0.99]"
          onClick={async () => {
            const { downloadAgendaPdf } = await import("@/lib/mylife/lib/exportReports");
        ...
        <button
          ...
          onClick={async () => {
            const { downloadAgendaXlsx } = await import("@/lib/mylife/lib/exportReports");
...
function AddEventModal({
  ...
  async function submit(e: React.FormEvent) {
    ...
    toast.success(`« ${ev.titre} » ajouté 📅`);
    setTitre(""); setDebut(""); setFin(""); setLieu(""); setDesc(""); setRec("once"); setRecEnd(""); setRappel("");
    onClose();
  }

  return (
    <LifeModal open={open} onClose={onClose} title="Nouvel événement">
...
function EditEventModal({
  ...
  return (
    <LifeModal open onClose={onClose} title="Modifier l’événement">
      <form onSubmit={submit} className="max-h-[70vh] space-y-3 overflow-y-auto pr-1">
        {inProgress && (
          <div>
