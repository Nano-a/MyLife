...
/** `nodeId` est-il un descendant (n’importe quelle profondeur) de `ancestorId` ? */
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="space-y-1">
        ...
        {folderPath.length > 0 && (
          <p
            className="truncate pl-0.5 text-[0.7rem] text-muted"
            title={["Notes", ...folderPath.map((s) => s.nom)].join(" › ")}
          >
...
function NoteEditor({
  ...
  const editor = useEditor({
    extensions: [
      StarterKit,
      Link.configure({ openOnClick: true, autolink: true, HTMLAttributes: { class: "underline text-primary" } }),
      Underline,
    ...
    >
      ...
      <div className="mb-3 flex flex-wrap gap-1 border-b border-current/10 pb-3">
        {[
          ...
          { label: "U",  tip: "Souligné",          action: () => editor?.chain().focus().toggleUnderline().run(),         active: editor?.isActive("underline") },
          { label: "🖍", tip: "Surlignage",        action: () => editor?.chain().focus().toggleHighlight().run(),         active: editor?.isActive("highlight") },
          { label: "🔗", tip: "Lien",              action: () => {
              const u = typeof window !== "undefined" ? window.prompt("URL du lien") : null;
...
function EditFolderModal({
  ...
  return (
    <LifeModal open={open} onClose={onClose} title="Modifier le dossier">
...
function CreateFolderModal({
  ...
  return (
    <LifeModal open={open} onClose={onClose} title="Nouveau dossier">
      <form onSubmit={submit} className="space-y-4">
        {/* Aperçu de l'icône */}
        <div className="flex justify-center">
          <div className="w-24">
