...
import { db, getProfile } from "@/lib/mylife/db";
...
import { useSportActiveStore } from "@/lib/sport-active-store";
...
function EndActiveSessionModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  async function save() {
    ...
    stop();
    toast.success(`Séance « ${s.libelle} » enregistrée 💪`);
    onClose();
  }

  return (
    <LifeModal open={open} onClose={onClose} title="Enregistrer la séance">
      <div className="space-y-4">
        <p className="text-sm text-muted">
...
function AddSessionModal({
  ...
  return (
    <LifeModal open={open} onClose={onClose} title={template ? `▶ ${template.nom}` : "Nouvelle séance"}>
      <form onSubmit={submit} className="space-y-4">
        ...
        <button
          ...
          onClick={() => {
            ...
            toast.success("Chrono démarré — reviens enregistrer quand tu as fini.");
            setNotes("");
            onClose();
          }}
...
function AddTemplateModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    toast.success(`Template « ${t.nom} » créé`);
    setNom(""); setDesc(""); onClose();
  }

  return (
    <LifeModal open={open} onClose={onClose} title="Nouveau template">
      <form onSubmit={submit} className="space-y-4">
        <div className="flex gap-2">
          <input autoFocus
